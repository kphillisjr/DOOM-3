...
public:
	...
	//------------------------
	virtual int32	GetProfileTag() {
		return -1;
	}

	int				GetDeviceNumForProfile() {
		return deviceNum;
	}
	...
	// settings
	//------------------------
	float			GetFrameScaleX() const {
		return frameScaleX;
	}
	float			GetFrameScaleY() const {
		return frameScaleY;
	}
	void			SetFrameScaleX( float scale ) {
		frameScaleX = scale;
	}
	void			SetFrameScaleY( float scale ) {
		frameScaleY = scale;
	}
	...
	bool			GetSeenInstallMessage() const {
		return seenInstallMessage;
	}
	void			SetSeenInstallMessage( bool seen ) {
		seenInstallMessage = seen;
	}

	bool			HasSavedGame() const {
		return hasSavedGame;
	}
	void			SetHasSavedGame() {
		hasSavedGame = true;
	}
